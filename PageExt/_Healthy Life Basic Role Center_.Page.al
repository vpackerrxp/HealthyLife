page 80200 "Healthy Life Basic Role Center"
{
    Caption = 'Healthy Life Basic Role Center';
    PageType = RoleCenter;
        ...
        area(sections)
        {
            group("Sales & Purchases")
            {
                ...
                action(SalesOrdersPartShipped)
                {
                    ...
                    RunObject = Page "Sales Order List";
                    RunPageView = WHERE(Status=FILTER(Released), "Completely Shipped"=FILTER(false));
                ...
                action(PurchaseOrdersPartReceived)
                {
                    ...
                    RunObject = Page "Purchase Order List";
                    RunPageView = WHERE(Status=FILTER(Released), "Completely Received"=FILTER(false));
