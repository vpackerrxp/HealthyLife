report 80200 "PC Remittance Advice - Journal"
{
    ...
    dataset
    {
        dataitem(FindVendors;"Gen. Journal Line")
        {
            ...
            trigger OnAfterGetRecord()begin
                if("Account Type" = "Account Type"::Vendor) and ("Account No." <> '')then if not TempVend.Get("Account No.")then begin
                        Vend.Get("Account No.");
                        if(Vend."EFT Bank Account No." <> '') and vend."EFT Payment" then VendorBankAccount.get(Vend."No.", vend."EFT Bank Account No.")
                        else
                        begin
                            if vend."Preferred Bank Account Code" <> '' then VendorBankAccount.get(Vend."No.", vend."Preferred Bank Account Code")
                            else
        ...
        dataitem(VendLoop;"Integer")
        {
            ...
            column(VendorBankAccountBSB;VendorBankAccount."Bank Branch No.")
            {
            }
            column(VendorBankAccountNo;VendorBankAccount."Bank Account No.")
            {
            }
            ...
            dataitem("Gen. Journal Line";"Gen. Journal Line")
            {
                ...
                dataitem("Vendor Ledger Entry";"Vendor Ledger Entry")
                {
                    ...
                    dataitem("Detailed Vendor Ledg. Entry";"Detailed Vendor Ledg. Entry")
                    {
                        ...
                        trigger OnAfterGetRecord()begin
                            ...
                            if "Vendor Ledger Entry No." <> "Applied Vend. Ledger Entry No." then InsertTempEntry(VendLedgEntry3);
                        end;
                    }
                    trigger OnAfterGetRecord()begin
                        InsertTempEntry("Vendor Ledger Entry")end;
                    trigger OnPreDataItem()begin
                        if "Gen. Journal Line"."Applies-to ID" = '' then CurrReport.Break();
                    end;
                }
                ...
                dataitem(PrintLoop;"Integer")
                {
                    ...
                    column(DocumentNo;TempAppliedVendLedgEntry."Document No.")
                    {
                    } //GS 14/04/2022
                    ...
                    trigger OnAfterGetRecord()begin
                        if Number = 1 then TempAppliedVendLedgEntry.Find('-')
                        else
                            TempAppliedVendLedgEntry.Next;
                        if JnlLineRemainingAmount < 0 then CurrReport.Skip();
                        ...
                        if TempAppliedVendLedgEntry."Currency Code" <> "Gen. Journal Line"."Currency Code" then begin
                            TempAppliedVendLedgEntry."Remaining Amount":=CurrExchRate.ExchangeAmtFCYToFCY("Gen. Journal Line"."Posting Date", TempAppliedVendLedgEntry."Currency Code", "Gen. Journal Line"."Currency Code", TempAppliedVendLedgEntry."Remaining Amount");
                            TempAppliedVendLedgEntry."Remaining Amount":=Round(TempAppliedVendLedgEntry."Remaining Amount", AmountRoundingPrecision);
                            TempAppliedVendLedgEntry."Amount to Apply":=CurrExchRate.ExchangeAmtFCYToFCY("Gen. Journal Line"."Posting Date", TempAppliedVendLedgEntry."Currency Code", "Gen. Journal Line"."Currency Code", TempAppliedVendLedgEntry."Amount to Apply");
                            ...
                            TempAppliedVendLedgEntry."Remaining Pmt. Disc. Possible":=CurrExchRate.ExchangeAmtFCYToFCY("Gen. Journal Line"."Posting Date", TempAppliedVendLedgEntry."Currency Code", "Gen. Journal Line"."Currency Code", TempAppliedVendLedgEntry."Original Pmt. Disc. Possible");
                            TempAppliedVendLedgEntry."Original Pmt. Disc. Possible":=Round(TempAppliedVendLedgEntry."Original Pmt. Disc. Possible", AmountRoundingPrecision);
                        end;
                        // Payment Discount
                        if("Gen. Journal Line"."Document Type" = "Gen. Journal Line"."Document Type"::Payment) and (TempAppliedVendLedgEntry."Document Type" in[TempAppliedVendLedgEntry."Document Type"::Invoice, TempAppliedVendLedgEntry."Document Type"::"Credit Memo"]) and ("Gen. Journal Line"."Posting Date" <= TempAppliedVendLedgEntry."Pmt. Discount Date") and (Abs(TempAppliedVendLedgEntry."Remaining Amount") >= Abs(TempAppliedVendLedgEntry."Remaining Pmt. Disc. Possible"))then PmdDiscRec:=TempAppliedVendLedgEntry."Remaining Pmt. Disc. Possible"
                        else
                            PmdDiscRec:=0;
                        ...
                        if TempAppliedVendLedgEntry."Remaining Amount" > 0 then if TempAppliedVendLedgEntry."Amount to Apply" < 0 then begin
                        ...
                        begin
                            if Abs(TempAppliedVendLedgEntry."Remaining Amount") > Abs(JnlLineRemainingAmount)then if TempAppliedVendLedgEntry."Amount to Apply" < 0 then PaidAmount:=Abs(TempAppliedVendLedgEntry."Amount to Apply")
                                else
                                    PaidAmount:=Abs(JnlLineRemainingAmount)
                            else if TempAppliedVendLedgEntry."Amount to Apply" < 0 then PaidAmount:=Abs(TempAppliedVendLedgEntry."Amount to Apply")
                                else
                        ...
                        // Numbers to print
                        if TempAppliedVendLedgEntry."Currency Code" <> "Gen. Journal Line"."Currency Code" then if PmdDiscRec <> 0 then PmdDiscRec:=PmtDiscInvCurr;
                        TempAppliedVendLedgEntry."Remaining Amount":=CurrExchRate.ExchangeAmtFCYToFCY("Gen. Journal Line"."Posting Date", "Gen. Journal Line"."Currency Code", TempAppliedVendLedgEntry."Currency Code", TempAppliedVendLedgEntry."Remaining Amount");
                    end;
            ...
            trigger OnAfterGetRecord()begin
                if Number = 1 then TempVend.Find('-')
                else
                    TempVend.Next;
    ...
    local procedure FindAmountRounding()begin
        if "Gen. Journal Line"."Currency Code" = '' then begin
            Currency.Init();
            Currency.Code:='';
            Currency.InitRoundingPrecision;
        end
        else if "Gen. Journal Line"."Currency Code" <> Currency.Code then Currency.Get("Gen. Journal Line"."Currency Code");
        AmountRoundingPrecision:=Currency."Amount Rounding Precision";
    end;
    local procedure InsertTempEntry(VendLedgEntryToInsert: Record "Vendor Ledger Entry")var AppAmt: Decimal;
    begin
        ...
        if TempAppliedVendLedgEntry.Insert()then begin
            ...
            if TempAppliedVendLedgEntry."Remaining Amt. (LCY)" > 0 then begin
                ...
                if "Gen. Journal Line"."Currency Code" <> '' then begin
                    AppAmt:=CurrExchRate.ExchangeAmtLCYToFCY("Gen. Journal Line"."Posting Date", "Gen. Journal Line"."Currency Code", AppAmt, "Gen. Journal Line"."Currency Factor");
