report 80205 "HL RemittanceAdvise"
{
            ...
            dataitem(VendLedgEntry2;"Vendor Ledger Entry")
            {
                ...
                dataitem("Detailed Vendor Ledg. Entry";"Detailed Vendor Ledg. Entry")
                {
                    ...
                    trigger OnAfterGetRecord()begin
                        ...
                        if "Vendor Ledger Entry No." = "Applied Vend. Ledger Entry No." then CurrReport.Skip();
                        VendLedgEntry3.CalcFields(Amount, "Remaining Amount");
                        LineAmount:=VendLedgEntry3.Amount - VendLedgEntry3."Remaining Amount";
                        LineDiscount:=CurrExchRate.ExchangeAmtFCYToFCY("Posting Date", '', "Currency Code", VendLedgEntry3."Pmt. Disc. Rcd.(LCY)");
                        LineDiscountCurr:=CurrExchRate.ExchangeAmtFCYToFCY(VendLedgEntry3."Posting Date", '', "Vendor Ledger Entry"."Currency Code", VendLedgEntry3."Pmt. Disc. Rcd.(LCY)");
                        VendLedgEntry3.Amount:=VendLedgEntry3.Amount + LineDiscountCurr;
                    end;
                }
                trigger OnAfterGetRecord()var DtldVendLedgEntry: Record "Detailed Vendor Ledg. Entry";
                begin
                    ...
                    if not DtldVendLedgEntry.FindFirst then CurrReport.Skip();
                    ...
                    LAmountWDiscCur:=-LineAmount - LineDiscount; //GS 31/03/2022
                    //LAmountWDiscCur := -LineAmount;
                    DocAmount:=-Amount;
                end;
    ...
    procedure CurrencyCode(SrcCurrCode: Code[10]): Code[10]begin
        if SrcCurrCode = '' then exit(GLSetup."LCY Code");
        exit(SrcCurrCode);
    end;
    local procedure FindApplnEntriesDtldtLedgEntry()var DtldVendLedgEntry1: Record "Detailed Vendor Ledg. Entry";
    ...
    begin
        ...
        if DtldVendLedgEntry1.Find('-')then begin
            repeat if DtldVendLedgEntry1."Vendor Ledger Entry No." = DtldVendLedgEntry1."Applied Vend. Ledger Entry No." then begin
                    ...
                    if DtldVendLedgEntry2.Find('-')then begin
                        repeat if DtldVendLedgEntry2."Vendor Ledger Entry No." <> DtldVendLedgEntry2."Applied Vend. Ledger Entry No." then begin
                                VendLedgEntry2.SetCurrentKey("Entry No.");
                                VendLedgEntry2.SetRange("Entry No.", DtldVendLedgEntry2."Vendor Ledger Entry No.");
                                if VendLedgEntry2.Find('-')then VendLedgEntry2.Mark(true);
                            end;
