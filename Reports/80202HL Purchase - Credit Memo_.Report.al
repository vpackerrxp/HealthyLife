report 80203 "PC Purchase - Credit Memo"
{
    ...
    dataset
    {
        dataitem("Purch. Cr. Memo Hdr.";"Purch. Cr. Memo Hdr.")
        {
            ...
            dataitem(CopyLoop;"Integer")
            {
                ...
                dataitem(PageLoop;"Integer")
                {
                    ...
                    dataitem("Purch. Cr. Memo Line";"Purch. Cr. Memo Line")
                    {
                        ...
                        trigger OnPreDataItem()var PurchCrMemoLine: Record "Purch. Cr. Memo Line";
                    ...
                    dataitem(VATCounterLCY;"Integer")
                    {
                        ...
                        trigger OnAfterGetRecord()begin
                            VATAmountLine.GetLine(Number);
                            VALVATBaseLCY:=VATAmountLine.GetBaseLCY("Purch. Cr. Memo Hdr."."Posting Date", "Purch. Cr. Memo Hdr."."Currency Code", "Purch. Cr. Memo Hdr."."Currency Factor");
                            VALVATAmountLCY:=VATAmountLine.GetAmountLCY("Purch. Cr. Memo Hdr."."Posting Date", "Purch. Cr. Memo Hdr."."Currency Code", "Purch. Cr. Memo Hdr."."Currency Factor");
                        end;
                        trigger OnPreDataItem()begin
                            if(not GLSetup."Print VAT specification in LCY") or ("Purch. Cr. Memo Hdr."."Currency Code" = '')then CurrReport.Break();
                            SetRange(Number, 1, VATAmountLine.Count);
                            Clear(VALVATBaseLCY);
                    ...
                    dataitem(Total;"Integer")
                    {
                        ...
                        trigger OnPreDataItem()begin
                            if "Purch. Cr. Memo Hdr."."Buy-from Vendor No." = "Purch. Cr. Memo Hdr."."Pay-to Vendor No." then CurrReport.Break();
                        end;
                    }
                ...
                trigger OnPostDataItem()begin
                    if not IsReportInPreviewMode then CODEUNIT.Run(CODEUNIT::"PurchCrMemo-Printed", "Purch. Cr. Memo Hdr.");
                end;
                trigger OnPreDataItem()begin
                    OutputNo:=1;
            ...
            trigger OnAfterGetRecord()begin
                ...
                else if GLSetup.GSTEnabled(PurchInvHeader."Document Date")then begin
                        AppliedToText:="Applies-to Doc. No.";
                        if PurchInvHeader.Get("Adjustment Applies-to")then OriginalInvDate:=PurchInvHeader."Document Date"
                        else
                            OriginalInvDate:=0D;
                    end
                    else
    ...
    trigger OnPostReport()begin
        if LogInteraction and not IsReportInPreviewMode then if "Purch. Cr. Memo Hdr.".FindSet then repeat SegManagement.LogDocument(16, "Purch. Cr. Memo Hdr."."No.", 0, 0, DATABASE::Vendor, "Purch. Cr. Memo Hdr."."Buy-from Vendor No.", "Purch. Cr. Memo Hdr."."Purchaser Code", '', "Purch. Cr. Memo Hdr."."Posting Description", '');
                until "Purch. Cr. Memo Hdr.".Next = 0;
    end;
    trigger OnPreReport()begin
        if not CurrReport.UseRequestPage then InitLogInteraction;
    end;
    var Text003: Label '(Applies to %1 %2)';
    ...
    local procedure DocumentCaption(): Text[250]begin
        if "Purch. Cr. Memo Hdr."."Prepayment Credit Memo" then exit(Text011);
        exit(Text005);
    end;
    ...
    local procedure FormatAddressFields(var PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr.")begin
    ...
    begin
            ...
            ReturnOrderNoText:=FormatDocument.SetText(PurchCrMemoHdr."Return Order No." <> '', PurchCrMemoHdr.FieldCaption(PurchCrMemoHdr."Return Order No."));
            ReferenceText:=FormatDocument.SetText(PurchCrMemoHdr."Your Reference" <> '', PurchCrMemoHdr.FieldCaption(PurchCrMemoHdr."Your Reference"));
            VATNoText:=FormatDocument.SetText(PurchCrMemoHdr."VAT Registration No." <> '', PurchCrMemoHdr.FieldCaption(PurchCrMemoHdr."VAT Registration No."));
    ...
    [IntegrationEvent(TRUE, false)]
    local procedure OnAfterPostDataItem(var PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr.")begin
    end;
}
